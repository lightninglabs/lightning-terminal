	...
	assertMigrationResults := func(t *testing.T, sqlStore *SQLDB,
		...
		for _, entry := range kvEntries {
			...
			if entry.groupAlias.IsNone() {
				...
				v, err := sqlStore.GetSessionGroupKVStoreRecord(
					ctx,
					sqlc.GetSessionGroupKVStoreRecordParams{
						Key:    entry.key,
						Perm:   entry.perm,
						RuleID: ruleID,
...
// mergeEntries is a helper function that merges multiple slices of kvEntry
// pointers into a single slice of kvEntry pointers.
func mergeEntries(entries ...[]*kvEntry) []*kvEntry {
	var result []*kvEntry

	for _, entrySlice := range entries {
		result = append(result, entrySlice...)
	}
...
func allEntryCombinations(t *testing.T, ctx context.Context, boltDB *BoltDB,
	...
	sEntries1 := sessionSpecificEntries(t, ctx, boltDB, sessionStore)
	fEntries1 := featureSpecificEntries(t, ctx, boltDB, sessionStore)
	...
	gEntries3 := insertTempAndPermEntry(
		t, ctx, boltDB, testRuleName2, fn.None[[]byte](),
		fn.None[string](), testEntryKey2, nilValue,
	)
	gEntries4 := insertTempAndPermEntry(
		t, ctx, boltDB, testRuleName2, fn.None[[]byte](),
		fn.None[string](), testEntryKey3, emptyValue,
	)

	sEntries3 := insertTempAndPermEntry(
		t, ctx, boltDB, testRuleName2, groupAlias,
		fn.None[string](), testEntryKey2, nilValue,
	)
	sEntries4 := insertTempAndPermEntry(
		t, ctx, boltDB, testRuleName2, groupAlias,
		fn.None[string](), testEntryKey3, emptyValue,
	)

	fEntries4 := insertTempAndPermEntry(
		t, ctx, boltDB, testRuleName2, groupAlias,
		fn.Some(testFeatureName), testEntryKey2, nilValue,
	)
	fEntries5 := insertTempAndPermEntry(
		t, ctx, boltDB, testRuleName2, groupAlias,
		fn.Some(testFeatureName), testEntryKey3, emptyValue,
	)

	return mergeEntries(
		gEntries1, gEntries2, gEntries3, gEntries4, sEntries1,
		sEntries2, sEntries3, sEntries4, fEntries1, fEntries2,
		fEntries3, fEntries4, fEntries5,
	)
}
...
func randomKVEntries(t *testing.T, ctx context.Context,
	...
	for i := 0; i < numberOfEntries; i++ {
		...
		entry := &kvEntry{
			...
			perm:        perm,
			// We'll generate a random value for all entries,
			value: []byte(randomString(rand.Intn(100) + 1)),
		}

		// 25% of the time, we will set the entry to a nil or empty
		// value.
		if rand.Intn(4) == 0 {
