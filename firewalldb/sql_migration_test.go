		...
		if rand.Intn(2) == 0 {
			for i := 1; i <= rand.Intn(5)+1; i++ {
				var (
					...
					// To ensure that the latest account
					// created in the loop is the one that
					// may be linked to the action, this
					// latest account expires later than any
					// previously created account in the
					// loop. The latest account will be the
					// that's linked to the action during
					// the migration, if multiple accounts
					// collide with the action's macaroon
					// identifier, and no session is set
					// for the action.
					expiry = time.Now().Add(
				...
				// expired, and if a session hasn't been set for
				// the action. When session has been set, the
				// session takes precedence over accounts, so we
				// don't link the account.
				// Note that this linked account may be
				// overwritten later in the loop if a new
				// account is created, as the account with the
				// latest expiry should be the one linked to the
				// action.
