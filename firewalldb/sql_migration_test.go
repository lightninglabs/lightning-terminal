...
func actionNoSessionOrAccount(t *testing.T, ctx context.Context,
	...
	actionReq.MacaroonIdentifier = fn.Some(rootKey)
	...
	// As the add action function doesn't return an action with the same
	// values set for the SessionID and AccountID as in the actionReq,
	// we need to overwrite the values returned by the action's store.
	// This is done as the action we return as the expected result should
	// match what we expect the migrated action to look like, which in this
	// case means having no session or account linked to the action.
	action.SessionID = fn.None[session.ID]()
	action.AccountID = fn.None[accounts.AccountID]()
...
func actionWithSessionNoAccount(t *testing.T, ctx context.Context,
	...
	rootKey := rStore.addRootKeyFromIDPrefix(sess.ID)
	...
	action := addAction(t, ctx, boltDB, actionReq)
	action.SessionID = fn.Some(sess.ID)
	action.AccountID = fn.None[accounts.AccountID]()
...
func actionsWithFilteredSession(t *testing.T, ctx context.Context,
	...
	// As the actions added by this function are not expected to be linked
	// to any session or account, we also set those values to None before
	// appending the action to the slice.
	addActionFromReq := func(req *AddActionReq) {
		action := addAction(t, ctx, boltDB, req)
		action.SessionID = fn.None[session.ID]()
		action.AccountID = fn.None[accounts.AccountID]()
	...
	rootKey1 := rStore.addRootKeyFromIDPrefix(sess1.ID)

	actionReq1 := &testActionReq
	actionReq1.MacaroonIdentifier = fn.Some(rootKey1)
	...
	rootKey2 := rStore.addRootKeyFromIDPrefix(sess2.ID)
	actionReq2 := &testActionReq
	actionReq2.MacaroonIdentifier = fn.Some(rootKey2)
...
func actionsWithFilteredAccount(t *testing.T, ctx context.Context,
	...
	actionReq3.MacaroonIdentifier = fn.Some(rootKey3)
	actionReq3.SessionID = fn.None[session.ID]()
	actionReq3.AccountID = fn.Some(acct3.ID)
	...
	actionReq4.MacaroonIdentifier = fn.Some(rootKey4)
	actionReq4.SessionID = fn.None[session.ID]()
	actionReq4.AccountID = fn.Some(acct4.ID)
...
func actionWithMultipleAccounts(t *testing.T, ctx context.Context,
	...
	_, acctID2 := testAccountWithExpiry(
		t, ctx, acctStore, time.Now().Add(time.Hour*2),
	)
	...
	actionReq.MacaroonIdentifier = fn.Some(rootKey)
	actionReq.SessionID = fn.None[session.ID]()
	// When two colliding accounts exist, the account with the latest expiry
	// should be linked to the action. In our case, that's acct2.
	actionReq.AccountID = fn.Some(newAcct2ID)

	action := addAction(t, ctx, boltDB, actionReq)
	action.SessionID = fn.None[session.ID]()
	action.AccountID = fn.Some(newAcct2ID)
...
func actionWithSessionWithLinkedAccountAndAccount(t *testing.T,
	...
	action := addAction(t, ctx, boltDB, actionReq)
	action.SessionID = fn.Some(sess.ID)
	action.AccountID = fn.Some(acct1.ID)
...
func randomActions(t *testing.T, ctx context.Context, boltDB *BoltDB,
	...
	for i := 0; i < numActions; i++ {
		...
		rpcMethod := randomString(rand.Intn(10) + 1)
		...
		actionReq := AddActionReq{
			...
			MacaroonIdentifier: fn.None[[4]byte](),
			SessionID:          fn.None[session.ID](),
			AccountID:          fn.None[accounts.AccountID](),
		}

		// 50% of the time, we create a session that may be linked to
		// the action.
		if rand.Intn(2) == 0 {
			switch rand.Intn(3) {
			...
			case 0:
				...
				rootKey := rStore.addRootKeyFromIDPrefix(
					sess.ID,
				)
				actionReq.MacaroonIdentifier = fn.Some(rootKey)
				actionReq.SessionID = fn.Some(sess.ID)
			...
			case 1:
				...
				actionReq.MacaroonIdentifier = fn.Some(
					rStore.addRootKeyFromIDPrefix(sess.ID),
				)
		...
		// 50% of the time, we create one or more accounts that may be
		...
		if rand.Intn(2) == 0 {
			for i := 0; i < rand.Intn(5)+1; i++ {
				...
				filtered := rand.Intn(2) == 0

				// In 50% of the cases, we create an account
				// that will be filtered out during the
				// migration, and therefore not be linked to the
				// action.
				if filtered {
					acct, acctID, actionReq = randFilteredAcct(
						t, ctx, acctStore, actionReq,
					)
				} else {
					// Else we create an account that is
					// active, and that may be linked to the
					// action. To ensure that the latest
					// account created in the loop is the
					// one that may be linked to the action,
					// this account expires later than any
					// previously created account in the
					// loop. Note that the account is only
					// linked to the action if no session
				...
				if actionReq.MacaroonIdentifier.IsNone() {
					actionReq.MacaroonIdentifier = fn.Some(
					...
					rootKey := actionReq.MacaroonIdentifier.
						UnwrapOrFail(t)
				...
				// If a session hasn't been set for the action,
				// we link the action. If a session has been
				// set, the session takes precedence over
				// accounts, so we don't link the account.
				// If the account shouldn't be filtered, we know
				// for sure that this account is the one that
				// should be linked to the action, as it's the
				// account with the latest expiry.
				if actionReq.SessionID.IsNone() && !filtered {
					actionReq.AccountID = fn.Some(acct.ID)
				}
			}
		}

		// If the action doesn't have a MacaroonIdentifier yet, that
		// means no session or account was created for the action.
		// In that scenario, we create a random root key to use as the
		// MacaroonIdentifier, to simulate an action that was created
		// without any session or account linked to it.
		if actionReq.MacaroonIdentifier.IsNone() {
			actionReq.MacaroonIdentifier = fn.Some(
		...
		// Set the actions session and account IDs to match what we
...
// passed AddActionReq and returns the added action.
...
// randFilteredSession creates and returns an account that will be filtered out
// during the actions migration. The exact reason why the account will be
// filtered out is random.
func randFilteredAcct(t *testing.T, ctx context.Context,
	acctStore accounts.Store, currentReq AddActionReq) (
	*accounts.OffChainBalanceAccount, int64, AddActionReq) {
