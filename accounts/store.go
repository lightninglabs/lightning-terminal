...
// of a user's off-chain account balance. This balance can only be spent
// by paying invoices.
...
// Store wraps the bolt DB that stores all accounts and their balances.
type Store struct {
	db *bbolt.DB
}

// NewStore creates an Store instance and the corresponding bucket in the bolt
// DB if it does not exist yet.
func NewStore(db *bbolt.DB) (*Store, error) {
	...
	// Return the DB wrapped in a Store object.
...
// randomly chosen ID.
func (s *Store) NewAccount(balance lnwire.MilliSatoshi,
...
// if it exists.
func (s *Store) storeAccount(account *OffChainBalanceAccount) error {
...
// yet exist in the DB.
func (s *Store) uniqueRandomAccountID() (AccountID, error) {
	var newID = AccountID{}
...
// GetAccount retrieves an account from the bolt DB and un-marshals it. If the
...
// GetAccounts retrieves all accounts from the bolt DB and un-marshals them.
func (s *Store) GetAccounts() ([]*OffChainBalanceAccount, error) {
...
func (s *Store) RemoveAccount(id AccountID) error {
...
func (s *Store) checkBalance(id AccountID,
...
func (s *Store) chargeAccount(id AccountID, paymentHash lntypes.Hash,
	...
	account.Payments[paymentHash] = struct{}{}
...
// paid.
func (s *Store) associateInvoice(id AccountID, hash lntypes.Hash) error {
	account, err := s.GetAccount(id)
	if err != nil {
		return err
	}
...
// amount credited.
func (s *Store) CreditAccount(paidInvoice lntypes.Hash,
	amount lnwire.MilliSatoshi) error {
	...
	for _, account := range accounts {
		...
		for associatedInvoice := range account.Invoices {
			if associatedInvoice != paidInvoice {
				continue
			}
...
func serializeAccount(account *OffChainBalanceAccount) ([]byte, error) {
	...
	for invoiceHash := range account.Invoices {
		if err := lnwire.WriteElement(&w, invoiceHash[:]); err != nil {
			return nil, err
		}
	}

	for paymentHash := range account.Payments {
		if err := lnwire.WriteElement(&w, paymentHash[:]); err != nil {
			return nil, err
		}
	}
...
func deserializeAccount(content []byte) (*OffChainBalanceAccount, error) {
	...
	account.Invoices = make(map[lntypes.Hash]struct{}, numInvoices)
	...
	account.Payments = make(map[lntypes.Hash]struct{}, numPayments)
