...
type AccountPaymentsRequest struct {
	...
	// determine which payments should be returned in the response, where the
	// index_offset is excluded. The payments are returned in ascending
	...
	// in the response.
	CountTotalPayments bool `protobuf:"varint,6,opt,name=count_total_payments,json=countTotalPayments,proto3" json:"count_total_payments,omitempty"`
	unknownFields      protoimpl.UnknownFields
	sizeCache          protoimpl.SizeCache
}
...
type AccountPaymentsResponse struct {
	...
	// The index of the first payment returned. This can be used as the
	// index_offset in a subsequent query to paginate backwards.
	...
	// The index of the last payment returned. This can be used as the index_offset
	// in a subsequent query to paginate forwards.
...
const file_lit_accounts_proto_rawDesc = "" +
	...
	"\x14count_total_payments\x18\x06 \x01(\bR\x12countTotalPayments\"\xcd\x01\n" +
