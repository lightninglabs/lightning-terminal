...
var (
	...
	// defaultRemoteLndMacPath is the default path we assume for a local lnd
	// node to store its admin macaroon file.
	defaultRemoteLndMacPath = filepath.Join(
	...
	return &Config{
		...
		Remote: &RemoteConfig{
			...
			Lnd: &RemoteDaemonConfig{
				...
				MacaroonPath: defaultRemoteLndMacPath,
...
func validateRemoteModeConfig(cfg *Config) error {
	...
	if r.Lnd.Network != defaultNetwork &&
		r.Lnd.MacaroonPath == defaultRemoteLndMacPath {

		r.Lnd.MacaroonPath = filepath.Join(
			lndDefaultConfig.DataDir, defaultLndChainSubDir,
			defaultLndChain, r.Lnd.Network, defaultLndMacaroon,
		)
	}
