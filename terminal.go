...
func (g *LightningTerminal) Run(ctx context.Context) error {
	...
	// Create litd's own shutdown source and make it available to the
	// package-level logger so that critical logs can bring litd down. It
...
func (g *LightningTerminal) waitForShutdown(ctx context.Context,
	...
	for {
		...
		case <-lndQuit:
			...
			// teardown, mark LiT as stopped so the status endpoint
