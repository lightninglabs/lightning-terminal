...
// TestValidateSuperMacaroonPath tests that ValidateSuperMacaroonPath correctly
// checks the suffix of the super macaroon path.
func TestValidateSuperMacaroonPath(t *testing.T) {
	t.Parallel()
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			err := ValidateSuperMacaroonPath(tt.path)
			if tt.wantErr {
				require.Error(t, err)
				require.Contains(
					t, err.Error(),
					"super-macaroon-path must end with "+
						"the .macaroon suffix",
				)
			} else {
				require.NoError(t, err)
