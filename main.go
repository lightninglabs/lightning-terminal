...
func main() {
	...
	if *runHttpServer {
		// Debug server.
		debugServer := buildServer(wrappedGrpc, fs)
	...
	if *runTlsServer {
		// Debug server.
		servingServer := buildServer(wrappedGrpc, fs)
...
func buildServer(wrappedGrpc *grpcweb.WrappedGrpcServer, fs http.Handler) *http.Server {
	return &http.Server{
		WriteTimeout: *flagHttpMaxWriteTimeout,
		ReadTimeout:  *flagHttpMaxReadTimeout,
		Handler: http.HandlerFunc(func(resp http.ResponseWriter, req *http.Request) {
			if wrappedGrpc.IsGrpcWebRequest(req) {
				logrus.Info("Handle GRPC request: ", req.URL.Path)
				wrappedGrpc.ServeHTTP(resp, req)
			} else {
				// fallback to static file hosting
...
func buildGrpcProxyServer(logger *logrus.Entry) *grpc.Server {
	...
	director := func(ctx context.Context, fullMethodName string) (context.Context, *grpc.ClientConn, error) {
		...
		// hard-code admin macaroon in hex
		mdCopy.Append("macaroon", "")
