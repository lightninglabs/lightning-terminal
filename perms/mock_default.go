...
//
// NOTE: This is the default build variant that does not populate sub-server
// specific config fields, since those fields only exist when the corresponding
// build tags (autopilotrpc, chainrpc, signrpc, walletrpc) are set. Without
// those tags, the sub-servers don't register, so these FetchConfig cases are
